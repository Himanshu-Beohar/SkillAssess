...
const assessmentsPage = {
  async load() {
    ...
    // ✅ 4. Generate filter options dynamically
    const categories = [...new Set(assessments.map(a => a.category).filter(Boolean))];
    const jobRoles = [...new Set(assessments.map(a => a.job_role).filter(Boolean))];
  ...
  renderAssessmentCard(assessment) {
    ...
    if (assessment.trending) {
      badgeText = "Trending";
      badgeIcon = "🔥";
    } else if (assessment.attempts > 500) {
      badgeText = "Popular";
      badgeIcon = "📈";
    }
    ...
    <div class="assessment-card" 
      ...
      <div class="assessment-content">
        ...
        <!-- 📊 New Assessment Info Section -->
        <div class="assessment-info">
          <div class="info-item">
            <i class="fas fa-list-ol"></i>
            <span>${assessment.num_questions || 0} Questions</span>
          </div>
          <div class="info-item">
            <i class="fas fa-clock"></i>
            <span>${assessment.time_limit ? assessment.time_limit + ' min' : 'No limit'}</span>
          </div>
          <div class="info-item difficulty-${(assessment.difficulty || 'Beginner').toLowerCase()}">
            <i class="fas fa-signal"></i>
            <span>${assessment.difficulty || 'Beginner'}</span>
          </div>
        </div>
  ...
  addEventListeners() {
    ...
    // ✅ Category, Difficulty & Job Role
    const categorySelect = document.getElementById("category-filter");
    const difficultySelect = document.getElementById("difficulty-filter");
    const jobRoleSelect = document.getElementById("jobrole-filter");
    if (categorySelect) categorySelect.addEventListener("change", this.applyAdvancedFilters.bind(this));
    if (difficultySelect) difficultySelect.addEventListener("change", this.applyAdvancedFilters.bind(this));
    if (jobRoleSelect) jobRoleSelect.addEventListener("change", this.applyAdvancedFilters.bind(this));
  },
  ...
  applyAdvancedFilters() {
    const category = document.getElementById("category-filter")?.value || "";
    const difficulty = document.getElementById("difficulty-filter")?.value || "";
    const jobRole = document.getElementById("jobrole-filter")?.value || "";
    const searchTerm = document.getElementById("search-input")?.value.toLowerCase() || "";
    const urlParams = new URLSearchParams(window.location.search);
    const skillFilter = urlParams.get("skill");
    const activeToggle = document.querySelector(".toggle-btn.active")?.getAttribute("data-value") || "all";
    ...
    cards.forEach(card => {
      ...
      const cardDifficulty = card.getAttribute("data-difficulty") || "";
      const cardSkill = card.getAttribute("data-skill") || "";
      const cardJobRole = card.getAttribute("data-job-role") || "";
      const isPremium = card.getAttribute("data-is-premium") === "true";
      ...
      // 🎯 Skill filter
      if (skillFilter && cardSkill.toLowerCase() !== skillFilter.toLowerCase()) show = false;
      ...
      // 💼 Job Role filter
      if (jobRole && cardJobRole !== jobRole) show = false;
